...
* `token`：个人访问令牌（personal access token），使用 Secret `github-token` 的 `token` 键的值，该字段是可选的。
...
* `token`：Hugging Face token，使用 Secret `hf-token` 的 `token` 键的值，该字段是可选的。
...
* `s3-endpoint`：S3 端点，使用 Secret `s3-config` 的 `endpoint` 键的值。
* `s3-access-key-id`：S3 服务的 AccessKeyID 凭证，使用 Secret `s3-config` 的 `accessKeyID` 键的值。
* `s3-secret-access-key`：S3 服务的 SecretAccessKey 凭证，使用 Secret `s3-config` 的 `secretAccessKey` 键的值。
* `s3-uri`：S3 文件或目录的路径。
